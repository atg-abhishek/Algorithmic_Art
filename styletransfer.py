"""
Initial version of the file based on the tutorial on style transfer from
https://github.com/walid0925/AI_Artistry/blob/master/main.py

UPDATE 1: initial changes made to adhere to flake8 linting
"""
...
cImArr = K.variable(preprocess_input(np.expand_dims(cImArr, axis=0)),
                    dtype='float32')

sImage = load_img(path=sImPath, target_size=targetSize)
sImArr = img_to_array(sImage)
sImArr = K.variable(preprocess_input(np.expand_dims(sImArr, axis=0)),
                    dtype='float32')

gIm0 = np.random.randint(256, size=(targetWidth, targetHeight, 3)).\
    astype('float64')
...
def get_total_loss(gImPlaceholder, alpha=1.0, beta=10000.0):
    F = get_feature_reps(gImPlaceholder, layer_names=[cLayerName],
                         model=gModel)[0]
    Gs = get_feature_reps(gImPlaceholder, layer_names=sLayerNames,
                          model=gModel)
...
def get_grad(gImArr):
    ...
    if gImArr.shape != (1, targetWidth, targetHeight, 3):
        gImArr = gImArr.reshape((1, targetWidth, targetHeight, 3))
    grad_fcn = K.function([gModel.input], K.gradients(
...
gModel = VGG16(include_top=False, weights='imagenet',
